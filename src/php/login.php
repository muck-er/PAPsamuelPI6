...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password)) {
        echo "<script>alert('Por favor, preencha todos os campos.');window.location.href='/public/html/login.html';</script>";
        exit();
    }
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $user["password"])) {
            ...
            header("Location: /src/php/def_objectives.php");
            exit();
        } else {
            echo "<script>alert('Senha incorreta!');window.location.href='/public/html/login.html';</script>";
        }
    } else {
        echo "<script>alert('Email não encontrado!');window.location.href='/public/html/login.html';</script>";
    }
